...
def check_function_name(mach_1_file, line_number):
    """
    Extracts a line from mach-1 txt file 

    Args:
        mach_1_file (str): .txt file name generated by Mach-1 Motion 
        line_number (int): Line number to extract info

    Returns:
        Info (str): Information linked to the line number
    """
    with open(mach_1_file, 'r') as file:
        lines = file.readlines()
        return lines[line_number - 1].strip()

def find_dividers_in_range(mach_1_file, dividers, start_row, end_row):
    """
    Extracts range of lines from mach-1 file

    Args:
        mach_1_file (str): .txt file name generated by Mach-1 Motion 
        dividers (list): List of strings containing all dividers used in mach-1 .txt file
        start_row (int): Start range to look in file
        end_row (int): End range to look in file

    Returns:
        results (list): List of indexes of separators
    """
    results = []
    with open(mach_1_file, 'r') as file:
...
def read_mach_1_file(mach_1_file, read_data = 1, data_headers_selection = None, showProgressBar = True):
    ...
    dividers_indentation = ["Contacts mapping (x, y, z)","Surface Normal (x, y, z)"]
    ...
    for function in range(len(idxs_dividers)//4):
        ...
        funcName = check_function_name(mach_1_file, idxs_dividers[4*function+1] + 1).split("<")[-1].split(">")[0] 
        if funcName == "Normal Indentation":
            idxs_indentation = find_dividers_in_range(mach_1_file, dividers_indentation, idxs_dividers[4*function], idxs_dividers[4*function+2])
            dfFUNCTION = pd.read_csv(mach_1_file, sep="\t", skiprows = idxs_dividers[4*function+1], nrows=10, engine="c", na_filter=False, low_memory=False).to_dict()
            dfFUNCTION["<" + funcName + ">"][dividers_indentation[0]] = pd.read_csv(mach_1_file, skiprows = idxs_indentation[0], nrows=idxs_indentation[1] - idxs_indentation[0] - 1, sep='\s+', names=["x", "y", "z"], engine="c", na_filter=False, low_memory=False).values
            dfFUNCTION["<" + funcName + ">"][dividers_indentation[1]] = pd.read_csv(mach_1_file, skiprows = idxs_indentation[1], nrows=1, sep='\s+', names=["x", "y", "z"], engine="c", na_filter=False, low_memory=False).values
            dfFUNCTION["<" + funcName + ">"]["Angle from z-axis, deg:"] = pd.read_csv(mach_1_file, sep="\t", skiprows = idxs_indentation[1]+1, nrows=1, index_col = 0, header=None, engine="c", na_filter=False, low_memory=False).values[0,0]
            dfFUNCTION["<" + funcName + ">"]["Surface Normal mse"] =  pd.read_csv(mach_1_file, skiprows = idxs_indentation[1]+2, nrows=1, sep='\t', index_col = 0, header=None, engine="c", na_filter=False, low_memory=False).values[0,0]
            dfFUNCTION["<" + funcName + ">"]["Surface Normal is valid"] =  pd.read_csv(mach_1_file, skiprows = idxs_indentation[1]+3, nrows=1, sep='\t', index_col = 0, header=None, engine="c", na_filter=False, low_memory=False).values[0,0]
        else:
